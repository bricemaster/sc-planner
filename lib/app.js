...
// Guard prevents desktop↔mobile budget sync from triggering a feedback loop
...
function initOnboarding() {
  ...
  function goToStep(n) {
    ...
    dots.forEach(d => {
      ...
      if (dn < n) d.classList.add('visited');
    });
  ...
  ['onboardSolo', 'onboardSkill', 'onboardRisk'].forEach(id => {
    ...
    container.querySelectorAll('.onboard__tbtn').forEach(btn => {
      btn.addEventListener('click', () => {
        container.querySelectorAll('.onboard__tbtn').forEach(b => { b.classList.remove('active'); b.setAttribute('aria-checked', 'false'); });
...
function setupToggles(selector, stateKey, transform) {
  ...
  // Clear all active states first, then set the correct one from state
...
function syncToggleToMobile(desktopSelector, activeVal) {
  ...
  mobileBtns.forEach(b => {
    const isActive = b.dataset.val === activeVal;
    b.classList.toggle('active', isActive);
    b.setAttribute('aria-checked', isActive ? 'true' : 'false');
  });
}
...
function initTabOverlay() {
  ...
  function openTab(name) {
    const wasAlreadyOpen = overlay.classList.contains('open');
    ...
    // Inject context bar at top of tab panel (persists across tab switches)
    ...
    // Only trigger overlay open animation if not already open
    if (!wasAlreadyOpen) {
      overlay.classList.add('open');
      closeBtn.classList.add('visible');
    }

    // Re-trigger fade animation on visible tab content when switching
    if (wasAlreadyOpen) {
      const activeSection = document.getElementById('tab-' + name);
      if (activeSection) {
        const section = activeSection.querySelector('.tab-content-section') || activeSection;
        section.style.animation = 'none';
        section.offsetHeight; // Intentional forced reflow — required to restart CSS animation
...
function initLiveClock() {
  ...
  // Pause clock when tab is hidden to avoid unnecessary timers
  document.addEventListener('visibilitychange', () => {
    if (document.hidden) {
      clearInterval(clockInterval);
    } else {
      tick(); // update immediately on return
...
function initSectionFlash() {
  document.addEventListener('click', e => {
    ...
    if (target) {
      ...
      // Intentional forced reflow — required to restart CSS animation
