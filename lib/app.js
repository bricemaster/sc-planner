...
function renderShipsOnly() {
  ...
  // Build role filter pills into #shipFilters
  if (filtersEl) {
    let filterHtml = '';
  ...
  if (searchInput && searchInput.value !== state.shipSearch) {
    // Sync the input if state was changed programmatically
  ...
  filtered.forEach(ship => {
    ...
    // Key stat: pick the most notable nonzero stat
    ...
    const maxShipPrice = Math.max(...filtered.map(s => s.price));
